/*
 * heatsim_v2.c -- HeatSim version 2 (cache-friendly, sequential)
 * Make It Parallel, companion code (Chapter 5).
 *
 * Identical results to version 1, bit for bit. Each sweep over the
 * grid performs two time steps: the second step trails the first by
 * one row, so it reads rows that are still in the cache. For grids
 * too large for the caches, this cuts memory traffic per cell update
 * from about 24 bytes to about 16.
 *
 ...
 *
 * Build:  gcc -std=c17 -O2 -Wall -Wextra -o heatsim_v2 heatsim_v2.c
 * Run:    ./heatsim_v2 N STEPS [SNAPSHOT_STEP ...]
