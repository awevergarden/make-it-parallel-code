/*
 * heatsim_v3.c -- HeatSim version 3 (vectorized, sequential)
 * Make It Parallel, companion code (Chapter 6).
 *
 * Version 2 with one addition: "#pragma omp simd" asks the compiler
 * to vectorize the row update, which GCC's -O2 otherwise leaves
 * scalar. Each lane computes its own cell with the same operations
 * in the same order, so results are bit-identical to versions 1-2.
 *
 ...
 *
 * Build:  gcc -std=c17 -O2 -fopenmp-simd -Wall -Wextra \
 *             -o heatsim_v3 heatsim_v3.c
 *         (-fopenmp-simd is required; add -march=native for
 *         wider vectors on the machine you compile on)
 * Run:    ./heatsim_v3 N STEPS [SNAPSHOT_STEP ...]
...
/* Update one interior row i of dst from three rows of src. */
...
/* Two time steps in one sweep. The first step writes next from cur;
   the second, one row behind, writes cur from next. Cur's row i-1 is
   overwritten only after the first step has finished reading it. */
static void two_steps(double *cur, double *next, int n)
{
    for (int i = 1; i < n - 1; i++) {
        update_row(cur, next, i, n);
        if (i >= 2)
            update_row(next, cur, i - 1, n);
    }
    update_row(next, cur, n - 2, n);
}
...
int main(int argc, char *argv[])
{
    ...
    for (long s = 0; ; s++) {
        ...
        if (s == steps)
            break;
        if (s + 2 <= steps && snap_step != s + 1) {
            two_steps(cur, next, n);   /* cur now holds step s + 2 */
            s++;
            continue;
        }
        step(cur, next, n);
