...
# Waiting MPI processes must yield the processor: on machines that report
# more cores than they really have (such as cloud VMs), Open MPI otherwise
# spins, and the processes starve one another -- a test that takes a second
# can take minutes. Threads wait passively for the same reason.
MPIRUN="mpirun --oversubscribe --bind-to none --mca mpi_yield_when_idle 1"
